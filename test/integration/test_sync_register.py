"""Integration tests for POST /api/sync/register endpoint."""
import pytest
...
class TestSyncRegister:
    def test_register_new_client(self, client):
        ...
        response = client.post("/api/sync/register?client_id=new-client-001")
    ...
    def test_register_with_custom_name(self, client):
        ...
        response = client.post(
            "/api/sync/register?client_id=client-002&client_name=MyPhone"
        )
        assert response.status_code == 200
        data = response.json()
        assert data["status"] == "ok"
    ...
    def test_re_register_existing_client(self, client):
        ...
        # Register twice
        response1 = client.post(f"/api/sync/register?client_id={client_id}")
        assert response1.status_code == 200

        response2 = client.post(f"/api/sync/register?client_id={client_id}")
